...
// tbl_bbs에서 b_id 칼럼값을 데이터를 1개 select하고
// tbl_reply의 r_postId = b_id 칼럼으로 where 절을 실행하어
// tbl_reply를 select하고 그 list를 함꼐 묶어서 결과로 달라 
tbl_bbs.findOne({
  where:{b_id},
 ...
 * 
 * 1. queryString
 *  http://localhost:3000/bbs/delete?b_id=3 이렇게 보낼 때
 *  req.query.b_id로 받기
 *  const b_id = req.query.b_id
 * 
 * 2. body(form의 POST로 전송할 때)
